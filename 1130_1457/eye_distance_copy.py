...
UP_pin = 2                     # LED 핀은 라즈베리파이 GPIO 2번핀으로 
DOWN_pin = 17
GPIO.setup(UP_pin, GPIO.OUT,initial=GPIO.LOW)   # LED 핀을 출력으로 설정
...
try:
    while True:
        ...
        if time.time() - start_time >= 1:
            ...
            if( left_eye_height1/left_eye_length < 0.2 and left_eye_height1/left_eye_length < 0.2 ):
                sleep_stack+=1
                print("eye closed!")
                if(sleep_stack >= 3):
                    GPIO.output(UP_pin, GPIO.HIGH) # LED 핀에 HIGH 신호 인가(LED 켜짐)
                    print("action!")
                    time.sleep(2)                   
