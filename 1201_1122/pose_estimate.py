...
"""
def mouth_hand_algorithm_with_pose(pose_results, image):
    # 이미지 크기 가져오기
    ...
    # chin_landmarks_indices = [152, 365, 135, 149, 378]
    # chin_landmarks = [
    #     np.array([
    ...
    for hand in hand_positions:
        ...
        for mouth in mouth_positions:
            # print("point2")
            distance = np.linalg.norm(mouth - hand)  # 픽셀 단위 거리 계산
            
            # print(distance / shoulder_width)
            # 거리 임계값 설정 (픽셀 기준으로 비교)
            if distance / shoulder_width < 0.4:  # 비율 조정 가능
...
def is_chinrest(image):
    try:
        ...
        # 턱 괴기 탐지
        is_chin_resting = mouth_hand_algorithm_with_pose(pose_results, image)
        # 결과 표시
        if is_chin_resting:
            return True
        else:
